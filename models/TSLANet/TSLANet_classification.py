...
import dataloader
from TSLANet_dataloader import get_datasets
from TSLANet_utils import get_clf_report, save_copy_of_files, str2bool, random_masking_3D
...
def train_model(pretrained_model_path):
    ...
    trainer = L.Trainer(
        ...
        # logger=TensorBoardLogger(CHECKPOINT_PATH, name="train_logs")
    )
    ...
    if args.load_from_pretrained and pretrained_model_path:
        ...
        # Extract only the base model weights (remove 'model.' prefix)
        model_state_dict = {}
        for key, value in pretrained_state_dict.items():
            if key.startswith('model.'):
                # Remove 'model.' prefix to match the structure
                new_key = key[6:]  # Remove 'model.' (6 characters)
                model_state_dict[new_key] = value
        
        # Load weights into the base model only (not the whole training module)
...
if __name__ == '__main__':
    ...
    CHECKPOINT_PATH = f"/root/Mine_ROI_Net/checkpoints/TSLANet/seq_{SEQ_LEN}/{run_description}"
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=CHECKPOINT_PATH,
        save_top_k=0,          # Don't save based on metric monitoring
